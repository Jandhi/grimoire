...
def generate_districts(
    seed: int, build_rect: Rect, world_slice: WorldSlice, city_map: Map
) -> tuple[
    ...
    bubble_out(districts, district_map, city_map)
    ...
    establish_adjacency(world_slice, district_map, city_map)
    super_districts: list[SuperDistrict] = []
    super_district_map: list[list[District]] = [
        [None for _ in range(build_rect.size.y)] for _ in range(build_rect.size.x)
    ]
    for district in districts:
        district_analyze(district, city_map)
    ...
    establish_adjacency(world_slice, super_district_map, city_map)
    merge_down(super_districts, super_district_map, TARGET_DISTRICT_AMT, city_map)
