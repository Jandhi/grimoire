...
start = grid.get_door_coords(NORTH) + grid.grid_to_local(ivec3(0, 0, 0)) + ivec3(0, 0, 1)
...
def furnish_building(cells: list, door_coords: ivec3):
    ...
    for floor in range(num_floors):
        ...
        if floor == num_floors - 1:
            ...
            path = set(end)
            stairs_blocks, free_for_stairs = build_stairs(end, end_corner_type, floor_height)
            filled = filled.union(path)
