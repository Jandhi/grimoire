...
class Logger:
    ...
    def format(
        ...
        if self.settings.print_level:
            ...
            level_str = (
                f"{level.value.color}{level.name}{Style.reset}"
                if is_in_console
                else level.name
            )
    ...
    def __log(self, text: str, level: LoggingLevel):
        is_console_level = (
            level.value.rank >= self.settings.minimum_console_level.value.rank
        )
        if self.settings.print_to_console and is_console_level:
            print(self.format(text=text, level=level, is_in_console=True))
    ...
    def progress(
        self, iterable: Iterable, description: str, position: int = 0, leave=True
    ):
        output = self.format(
            text=description, level=LoggingLevel.INFO, is_in_console=True
        )
