...
def find_edges(points : set[ivec2]) -> set[ivec2]:
    return {
        point for point in points if any((point + get_ivec2(direction)) not in points for direction in cardinal)
    }
...
def find_outer_and_inner_points(points : set[ivec2], distance : int) -> tuple[set[ivec2], set[ivec2]]:
