...
from structures.legacy_directions import north, east, south, vector, ivec3_to_dir
...
def add_gates(wall_list : list, editor : Editor, world_slice : WorldSlice, is_thin : bool, inner_wall_dict : dict, palette : Palette, palisade : bool = False) -> list[Gate]:
        ...
        if palisade:
            ...
            if gate_possible == 0:
                ...
                if i<len(wall_list) - 7 and is_straight_not_diagonal_ivec2(ivec2(point.x,point.z), ivec2(wall_list[i+6][0], wall_list[i+6][2]), 6) and abs(point.y - wall_list[i+6][1]) <= 1:
                    ...
                    location = ivec3(middle_point.x, height_map[middle_point.x, middle_point.z],middle_point.z)
