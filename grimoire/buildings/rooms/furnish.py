...
from grimoire.core.utils.sets.find_outer_points import find_edges, find_edges_3D
...
def build_interior_walls(cells: list, floor: int, palette: Palette, editor: Editor, grid: Grid, rng: RNG) -> set:
    ...
    if len(cells_on_floor) == 1: # no interior walls if there is only 1 cell
        ...
        if len(cells_on_floor) > 4:
            #start with a random cell and a random direction
            starting_cell = rng.choose([c for c in cells_on_floor if c != starting_cell])
            possible_directions = []
...
def shift_end_for_stairs(end: ivec3, grid: Grid, cells: list, corner_type: str) -> ivec3:
    match corner_type:
        case 'southwest':
            end = end + ivec3(4, 0, 0)
        case 'northwest': 
...
def map_inside_area(grid: Grid, cells: list, floor: int):
    ...
    for point in find_edges(points):
        points.remove(point)
...
def get_blocks_along_wall(footprint: set, filled: set, dir: ivec3, editor: Editor) -> set:
    along_wall = set()
...
def furnish(footprint: set, filled: set, stairs_blocks: set, path_blocks: set, palette: Palette, editor: Editor, grid: Grid, rng: RNG) -> None:
    ...
    def furnish_along_wall(footprint: set, filled: set, stairs_blocks: set, editor: Editor) -> set:
        longest = 0
        longest_dir = None
        for dir in CARDINALS:
            blocks_along_dir_wall = get_blocks_along_wall(footprint, filled, dir, editor)
        ...
        along_wall_groups = get_blocks_along_wall(footprint, filled, longest_dir, editor)
...
def furnish_building(cells: list, door_coords: ivec3, door_dir: ivec3, palette: Palette, editor: Editor, grid: Grid, rng: RNG):
