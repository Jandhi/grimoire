...
cells = [
    ivec3(x, y, z) for x in range(x_size) for y in range(y_size) for z in range(z_size)
]
...
start = (
    grid.get_door_coords(NORTH) + grid.grid_to_local(ivec3(0, 0, 0)) + ivec3(0, 0, 1)
)
    ...
    match corner_type:
        case "southwest":
            for i in range(4):
                ...
                stairs.add(current)
                editor.placeBlock(
                    current + UP * floor_height, Block(id="minecraft:air")
                )
            ...
            build_nbt(
                ...
                stairs_nbt,
                palette,
                Transformation(offset=end + WEST + ivec3(0, 1, 0), rotations=1),
        ...
        case "northwest":
            ...
            build_nbt(
                ...
                stairs_nbt,
                palette,
                Transformation(offset=end + NORTH + ivec3(0, 1, 0), rotations=2),
                material_params_func=None,
                build_map=None,
            )
            current = end
        ...
        case "northeast":
            ...
            build_nbt(
                ...
                stairs_nbt,
                palette,
                Transformation(offset=end + EAST + ivec3(0, 1, 0), rotations=3),
                material_params_func=None,
                build_map=None,
            )
...
def get_blocks_along_wall(unfilled_blocks: set, stairs_blocks: set, dir: ivec3) -> set:
    along_wall = set()
    for block in unfilled_blocks:
        next = block + dir
        if next in stairs_blocks or (
            editor.getBlock(next + UP).id != "minecraft:air"
            and editor.getBlock(block + UP * 4).id == "minecraft:air"
        ):
    ...
    def furnish_along_wall(unfilled_blocks: set, stairs_blocks: set) -> set:
        ...
        for dir in CARDINALS:
            blocks_along_dir_wall = get_blocks_along_wall(
                unfilled_blocks, stairs_blocks, dir
            )
        ...
        along_wall_groups = get_blocks_along_wall(
            unfilled_blocks, stairs_blocks, longest_dir
        )
        ...
        for subdivision in plan:
            ...
            if rotate3D(legacy_directions.VECTORS[furniture.facing], 1) == longest_dir:
                build_nbt(
                    ...
                    furniture,
                    palette,
                    Transformation(offset=current + ivec3(0, 1, 0), rotations=1),
                    material_params_func=None,
                    build_map=None,
                )
...
def furnish_building(cells: list, door_coords: ivec3):
    ...
    for floor in range(num_floors):
        ...
        if floor == num_floors - 1:
            end, end_corner_type, floor_space = find_end(start, stairs=False)
            path = set(end)
            ...
            stairs_blocks, free_for_stairs = build_stairs(
                end, end_corner_type, floor_height
            )
            filled = filled.union(path)
