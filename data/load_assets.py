...
def load_assets(root_directory) -> None:
    ...
    for name in names:
        with open(name, "r") as file:
            ...
            if "type" not in data:
                print(
                    f"{Fore.red}Error{Style.reset}: could not load {path}. No type given."
                )
                continue
            ...
            if validation_state.is_invalid():
                print(
                    f"{Fore.red}Error{Style.reset}: while loading {Fore.light_blue}{path}{Style.reset}. Object is missing the following fields: {validation_state.missing_args}. It will be ignored."
                )
                continue
